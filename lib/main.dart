        ...
        content: SingleChildScrollView(
          child: Column(
            children: [
              ...
              DropdownButtonFormField<String>(initialValue: category, items: ['Main Equipment','Spares/Accessory','Consumable','Service','Other'].map((c) => DropdownMenuItem(value: c, child: Text(c))).toList(), onChanged: (v) => category = v ?? category, decoration: const InputDecoration(labelText: 'Category')),
              DropdownButtonFormField<String>(initialValue: status, items: ['DC','Billed','Pending','Returned','Other'].map((s) => DropdownMenuItem(value: s, child: Text(s))).toList(), onChanged: (v) => status = v ?? status, decoration: const InputDecoration(labelText: 'Status')),
            ],
          ),
        ),
        actions: [
